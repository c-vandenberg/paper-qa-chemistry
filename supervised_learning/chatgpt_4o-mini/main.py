...
def main():
    try:
        with open('data/processed/supervised_learning_gpt_4o-mini.pkl', 'rb') as file:
    ...
    papers: Generator = zotero.iterate(limit=20)
    ...
    for i, paper in enumerate(tqdm(papers, desc="Processing Papers", ncols=100, miniters=1, mininterval=0.5)):
        if paper.key in docs.docnames:
            print(f"Skipping already processed paper {i + 1}: {paper.key}")
            continue

        print(f"Processing paper {i + 1}: {paper.key}")
        ...
        with open('data/processed/supervised_learning_gpt_4o-mini.pkl', 'wb') as file:
